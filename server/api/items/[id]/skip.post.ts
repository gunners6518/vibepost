export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    if (actionError) {
      ...
      console.error('[Skip API] Failed to log action:', actionError)
    }
  } catch (err) {
    // Log but don't fail if action logging fails
    console.error('[Skip API] Failed to log action:', err)
  }
...
async function recalculateItemScore(supabase: any, itemId: string) {
  try {
    ...
    console.error('[Skip API] Failed to recalculate score:', err)
  }
}
